...
const codeVariants = cva('relative rounded-md bg-muted font-mono text-sm font-medium', {
  variants: {
    variant: {
      ...
      custom: 'bg-transparent text-white font-regular text-2xl',
    },
    size: {
      default: 'text-sm px-2.5 py-1.5',
      sm: 'text-xs px-2 py-1.5',
      lg: 'text-base px-3 py-1.5',
      custom: 'lg:text-2xl text-xl px-3 py-1.5',
    },
  },
  defaultVariants: {
    variant: 'default',
  ...
  return (
    <span className={cn('inline-flex items-center gap-2', className)} data-slot="code">
      ...
      {showCopyButton && textToCopy && (
        ...
        >
          {copied ? <Check className="h-3 w-3 stroke-white" /> : <Copy className="h-3 w-3 stroke-white" />}
        </Button>
      )}
    </span>
  );
